...
- **Dock tint:** the Dock takes on the album's colors as a soft, slowly drifting wash of the cover behind its glass. It follows an auto-hiding Dock, blends between songs, and dims while paused. Turn it off with **Tint Dock** in the ♪ menu.
...
For the Dock tint, allow Music Island under System Settings → Privacy & Security → **Accessibility**; it uses this only to find where the Dock is. Without it, the tint still works for a Dock that doesn't auto-hide.

`build.sh` signs each build with a new ad-hoc signature, and macOS ties the Accessibility permission to it. After rebuilding, reset it and allow it again:

```sh
tccutil reset Accessibility com.abhinav.MusicIsland
```
...
- **Dock tint:** apps can't draw inside the Dock, but the Dock's glass blurs whatever sits behind it. A click-through window one level below the Dock paints a translucent, blurred wash of the cover along the Dock's edge and a little stronger behind its glass, so the Dock still shows blurred windows through it, tinted. The Dock's frame comes from the Accessibility API. The idea comes from [Orb](https://github.com/nithish6541/orbdock) (MIT).
...
| `Sources/MusicIsland/DockGlow.swift` | The Dock tint: the window behind the Dock, the artwork wash, finding the Dock |
